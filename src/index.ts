...
const NOTFALL: Record<Language, Record<Kind, string>> = {
  de: {
    text: "Dieser Inhalt wurde mit künstlicher Intelligenz erzeugt.",
    bearbeitet:
      "Dieser Inhalt wurde mit künstlicher Intelligenz erzeugt und anschließend redaktionell bearbeitet.",
  ...
  en: {
    ...
    bearbeitet:
      "This content was generated using artificial intelligence and subsequently edited.",
    deepfake:
      "This image or audio material has been artificially generated or manipulated.",
...
export async function disclose(
  ...
  if (options.apiKey) kopf["X-API-Key"] = options.apiKey;
...
export async function verify(
  ...
  try {
    ...
    ergebnis = (await antwort.json()) as Record<string, unknown>;
  } catch (e) {
    ergebnis = {
      valid: null,
  ...
  if (content !== undefined) {
    const soll = (record as any)?.provenance?.content_sha256;
 ...
 */
export function wrap<T extends object>(client: T, options: DiscloseOptions = {}): T {
  const ziel = findeAufruf(client);
  if (!ziel) {
    throw new TypeError(
      "client shape not recognised - expected .chat.completions.create or " +
        ".messages.create. Use disclose() directly instead.",
    );
  }
  const [halter, name] = ziel;
  const original = (halter as any)[name];
  if (original.__eucomplianceWrapped) return client;

  const gewickelt = async function (this: unknown, ...args: any[]) {
  ...
  (gewickelt as any).__eucomplianceWrapped = true;
  (halter as any)[name] = gewickelt;
  return client;
}

function findeAufruf(client: any): [any, string] | null {
  const pfade = [
  ...
  for (const pfad of pfade) {
    ...
    if (ok && typeof o?.[letzter] === "function") return [o, letzter];
  }
  return null;
}

function textAus(antwort: any): string | null {
