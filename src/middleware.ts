 ...
 * `wrapStream` kennt.
 */
import { disclose, type DiscloseOptions, type Disclosure } from "./index.js";
...
function textAus(ergebnis: any): string {
  if (!ergebnis) return "";
  if (typeof ergebnis.text === "string") return ergebnis.text;

  const teile = ergebnis.content;
  if (Array.isArray(teile)) {
    return teile
      .filter((t: any) => t && (t.type === "text" || typeof t.text === "string"))
      .map((t: any) => t.text || "")
      .join("");
  }
  return "";
...
function melden(o: MiddlewareOptions, text: string, modell?: string): Promise<Disclosure | null> {
  if (!text) return Promise.resolve(null);
  return disclose(text, { ...o, model: o.model || modell })
    .then((d) => {
      try { o.onDisclosure?.(d); } catch { /* Protokoll des Kunden darf nie stoeren */ }
      return d;
    })
    .catch(() => null);
}
 ...
 */
export function euComplianceMiddleware(options: MiddlewareOptions = {}): any {
  return {
    async wrapGenerate({ doGenerate, model }: any) {
      const ergebnis = await doGenerate();
      try {
        const modell = model?.modelId || (options as any).model;
    ...
    async wrapStream({ doStream, model }: any) {
      ...
      try {
        ...
        const durchreichen = new TransformStream({
          transform(teil: any, steuerung: any) {
            try {
